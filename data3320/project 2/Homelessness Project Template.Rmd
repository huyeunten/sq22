...
Most of the variables describe demographics.
...
The graph shows that the linear model with all the variables 
...
```
The residual graph is similar to the residual graph from the first seed. The points continue to be systemically structured in a curve. SOMETHING ABOUT THE LAST GRAPH!
...
Not all of the variables are significant. At the 0.05 level, high_housing_density, share_overcrowded_units_2016, suburban, share_HUD_units, migration_4_year_change, proportion_one_person_households, share_no_bachelors, percentage_excessive_drinking, and total_Jan_precipitation are significant. The model also includes demographic variables. In the actual vs predicted homeless rate model, more points are clustered above the x = y line. The predicted values are higher than the actual values. The RMSE is 11.85. It's close to the RMSE of the full model.
...
plot(final_res$.predictions[[1]]$rate_homeless,final_res$.predictions[[1]]$.pred, xlab = "Homeless rate", ylab = "Predicted homeless rate")
abline(0, 1)
