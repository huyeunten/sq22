...
summary(fit_all)
```
The most significant variables based on the p-values are total_Jan_precipitation, proportion_one_person_households, migration_4_year_change, and log_median_rent.
...
```
#WRITE SOMETHING HERE IDK WHAT THIS MEANS!
...
```{r}
split_80 <- initial_split(df_model, prop = 0.80, strata = rate_homeless)

#Extract the training and testing splits
df_train_80 <- training(split_80)
df_test_80 <- testing(split_80)

fit_80 <- lm(rate_homeless ~ ., data = df_train_80)
summary(fit_80)
plot(fit_80, 1)
predict_80 <- predict(fit_80, df_test_80)
plot(df_test_80$rate_homeless, predict_80, xlab = "Actual Homeless Rate", ylab = "Predicted Homeless Rate")
```
#CHECK SIGNIFICANT VALUES! AND WRITE!
...
# 
# for (i in 1:num_fold) {
#   reg_fit_best <- regsubsets(rate_homeless ~ ., data = df_train[folds != j,], nvmax = n)
...
# points(ind_cv, error_mean[ind_cv],col = "red",pch = 20)
```
Each measure has a different number of variables. Minimizing the Cp has 13 variables, minimizing the BIC has 10 variables, and maximizing the adjusted R squared has 22 variables. The average of these three is a model with 15 variables.
...
summary(lm_cp)
```
All of the variables are significant at the 0.05 level.


$\rightarrow$ Generate predictions of the ACT score in the testing data.

```{r}
cp_pred <- predict(lm_cp, df_test)
```
...
```{r}
plot(df_test$rate_homeless, cp_pred, xlab = "Actual Homeless Rate", ylab = "Predicted Homeless Rate")
```
...
```{r}
cp_rmse <- (df_test$rate_homeless - cp_pred)^2 %>%
  mean() %>%
  sqrt()
cp_rmse
```
The RMSE is 11.85. It's close to the RMSE of the full model.

##### BIC Model Analysis

```{r}
df_bic <- df_train %>%
  select(all_of(names(coef(regfit_all, ind_cp))[2:length(coef(regfit_all, ind_bic))]), "rate_homeless")
