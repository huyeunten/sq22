...
domain problem: estimate probability that a household will switch wells when wells are labeled as safe or unsafe, using information about the asrsenic levels and the household

data problem: supervised learning problem
- describe relationship between well-switching behavior and household information
...
arsenic level in household's well (hundred micrograms/liter)
...
Switch (whether or not household switched wells), arsenic (arsenic levels), distance (distance to a clean well), assoc (whether or not the household is involved in community organizations), edu (household's highest education level).
...
```
The range of the arsenic values is from 0.51 to 9.65 hundreds μg/L. The range of the distances is 0.387 to 340 meters. The range of education levels is from 0 to 17. 1737 housholds switched wells, which is roughly half of the 3020 observations.
...
Farther distances have a slightly higher chance of switching wells, but the difference between the two is not large.
...
Higher education levels indicate a slightly higher chance of switching.
...
Households that are not involved in community organizations have a slightly higher chance of switching wells, though all the numbers are not drastically different.
